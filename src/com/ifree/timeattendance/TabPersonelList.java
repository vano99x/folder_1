...
public class TabPersonelList extends Tab implements View.OnClickListener
{
	...
	//*********************************************************************************************
	//       Event Handler
	private onWorkerFound get_onWorkerFound()
	{
		onWorkerFound a = new onWorkerFound();
		a.arg1 = this;
		return a;
	}
	class onWorkerFound extends RunnableWithArgs { public void run()
	{
		TabPersonelList _this = (TabPersonelList)this.arg1;
		
		Object[] resultArr = (Object[])this.result;
		Personel[] arrPersonel = (Personel[])resultArr[0];
	...
	private void PersonelListItem_Selected(Personel personel)
	{
		...
		this._engine.set_CurrentPersonel( personel );
		UIHelper.Instance().tabPersonelInfo.IsShowCheckiedWorker = false;
		UIHelper.Instance().switchState(com.ifree.timeattendance.MainActivity.State.PERSONEL_INFO);
	}
